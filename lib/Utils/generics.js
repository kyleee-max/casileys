...
const casileysVersion = [2, 3000, 1039785632];
...
/**
 * utility that fetches latest casileys version from the master branch.
 * Use to ensure your WA connection is always on the latest version
 */
export const fetchLatestCasileysVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/kaels/casileys/main/lib/Defaults/index.js';
    try {
        ...
        if (!response.ok) {
            throw new Boom(`Failed to fetch latest Casileys version: ${response.statusText}`, { statusCode: response.status });
        }
...
/**
 * Stream errors generally provide a reason, map that to a casileys DisconnectReason
